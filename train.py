...
def main():
    ...
    use_class = 'char_list/hira_kana_kanji.txt'
    with open(use_class) as f:
        use_list = [s.strip() for s in f.readlines()]

    use_class = 'char_list/hira_kana_kanji_val.txt'
    with open(use_class) as f:
    ...
    task = Task.init(project_name='character_classifer_hira_kana_kanji',
                    task_name=run_name,
                    auto_connect_arg_parser=False)
    task.connect_configuration(config)

    if config["use_gen"] in "select" and config["num_use"] is not None:
        # リストを書き込み
        with open(os.path.join("experiment", run_name, 'train.txt'), mode='w') as f:
            f.write('\n'.join(gen_list))
            ...
            if i%int(BASE_NUM_ETL_DATA/config["batch_size"])==0:
                ...
                if accuracy >=best_acc:
                    ...
                    task.upload_artifact(
                        'local file',
                        artifact_object=os.path.join("experiment", run_name, 'report.txt'))
